...
Gino 的個人記帳系統。開工前先讀 `docs/規格書.md`（不能變的規則）與 `docs/實作計畫.md`（技術決定與分期）。
動到資料庫連線、PWA、登入之前，先讀 `docs/踩過的雷.md`，那些坑都查過了。
每次做完一段，把白話進度寫進根目錄 `Gino.md`（給 Gino 自己回頭看的，不是技術文件）。

## 不能踩的線

`docs/規格書.md` 第 2.2 節的資料慣例是 Gino 記帳好幾個月累積下來的，動到就是改壞：
